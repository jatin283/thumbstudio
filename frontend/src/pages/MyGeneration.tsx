                                    ...
                                    ${aspectClass} bg-black`}>
                                        {thumb.image_url ? (
                                            ...
                                            <div className="w-full h-full flex items-center justify-center text-sm text-zinc-400">
                                                {thumb.isGenerating ? 'Generating....' : ''}
                                            </div>
                                        )}

                                        {thumb.isGenerating && <div className="absolute inset-0 bg-black/50 utems-center justify-center text-sm
                                ...
                                <div onClick={(e)=>e.stopPropagation()} className="absolute bottom-2 right-2 max-sm:flex sm:hidden
                                 group-hover:flex gap-1.5">
                                    ...
                                    <Link target="_blank" to={`/preview?thumbnail_url=${thumb.image_url}&title=${thumb.title}`}>
                                        <ArrowUpRightIcon onClick={()=>handleDelete(thumb._id)}
                                        className="size-6 bg-black/50 p-1 rounded hover:bg-pink-600
                                        transition-all" />
                                    </Link>
